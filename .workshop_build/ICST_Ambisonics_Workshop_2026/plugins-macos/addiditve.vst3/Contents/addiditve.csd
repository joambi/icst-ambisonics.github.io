...
instr 10
  ; MIDI keyboard → Fund Hz (jeden k-Zyklus, nicht nur beim metro)
  ; midiin liest rohe MIDI-Events unabhängig vom Routing (standalone + plugin)
  kstatus, kchan, kdata1, kdata2  midiin
  knoteon  = (kstatus >= 144 && kstatus <= 159 && kdata2 > 0)  ? 1 : 0
  knoteoff = ((kstatus >= 128 && kstatus <= 143) || (kstatus >= 144 && kstatus <= 159 && kdata2 == 0)) ? 1 : 0
  if knoteon == 1 then
    gkMidiFreq  = cpsmidinn(kdata1)
    gkNoteCount = gkNoteCount + 1
    gkMidiActive = 1
  endif
  if knoteoff == 1 then
    gkNoteCount  = (gkNoteCount > 0 ? gkNoteCount - 1 : 0)
    gkMidiActive = (gkNoteCount > 0 ? 1 : 0)
  endif
