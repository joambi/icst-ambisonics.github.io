---
---
Institute for Computer Music and Sound Technology / (ICST) Zurich University of the Arts

* * *

### At the  ICST, the staff use [ Reaper](http://reaper.fm/) 6.79 and [Ardour](http://ardour.org/) 7

### We recommend you do so as well, especially if you want to work with ambisonics or multi-channel audio in general.

* * *

## List of Software there is installed on the ICST Mac Pro:

**Audio - Analyse:**

* AudioSculpt 3.4.6
* RX-7 iZotope
* Acousmographe
* EAnalysis
* iSpectrum
* iZotope
* Melodyne 4
* Praat
* Sonic Visualiser
* SPEAR

**Ambisonics:**

* ATK Ambisonics Tool Kit (Plugins)
* Ambisonics Externals for Max (ICST)
* Ambi Head (B-format to Binaural converter)
* ambiX plugin suite v.0.2.7
* BlueRipple (free plugins)
* FB360 Spatial Workstation 
* Harpex-B
* Panoramix-Ircam
* Soundfield Limited
* Sound Particles U
* Sound Trajectory
* Spat Revolution Flux
* Spat for Max
* mcfx v0.5.5 – multichannel audio plug-in suite
* IEM Plugin suite
* ICST Ambisonics Plugins

**Audio-Tools:**

* Audio Hijack
* Dante Controller
* Dante Via
* Dante Virtual Soundcard
* JackPilot
* qjackctl
* Loopback

**Audio-Instruments:**

* Native Instruments

**DAW:**

* Ableton Live 10 Suite 
* Adobe Audition CC 2018
* Ardour5.12
* Audacity
* Digital Performer 9.52
* Logic Pro X 10.3.0
* REAPER64 (newest)

**Audio-Programming:**

* Cabbage
* Csound
* CsoundQt-d-py-cs6
* Faust
* Grace
* Max 8.0.1
* Pd-0.49-1-vanille
* Processing
* Plogue Bidule 0.9759
* SuperCollider

**Audio-Editor:**

* Cecilia5
* GRM-Player
* GRM Tools Complete 
* HourGlass
* iZotope RX 7 Audio Editor
* SoundGrain
* SoundHack
* TwistedWave 1.2

**Audio-Plugins:**

* ambiX
* FabFilter-Bundle
* Flux-Session
* GRM Tools 
* Harpex
* IEM
* Melodyne
* PaulStretch
* SIR 2 Audiotools
* RX-7
* FabFilters
* Two Big Ears FB360 Spatialiser
* ToscA
* UdeM SpatGris
* Waves Mercury
* ZHdK Ambisonics Plugins
* ZOOM MS-Decoder

**IRCAM-Bundle**:

* ADMIX
* ANGUS
* ANTESCOFO
* AUDIOSCULPT
* CATART STANDALONE
* CLEESE
* DAVID
* DIPHONE STUDIO 4.2
* FORUM MAX APPS
* GESTURE & SOUND
* MAX SOUND BOX
* MODALYS
* MUBU
* MUSIQUE LAB 2
* NAJO MODULAR INTERFACE 2
* OMAX
* OPENMUSIC
* ORCHIDS
* PANORAMIX
* SPAT
* SUPERVP FOR MAX
* SUPERVP TRAX
* TOSCA

**Notation:**

* Finale 25.0
* Sibelius
* INScore-1.21
* JMSL 
* MuseScore
* NoteAbilityPro
* OM-6.14-macOS
* PWGL
* Scala

**OSC-Tools:**

* IanniX
* INScore-1.21
* ToscA

* * *

<< [ICST Kompositionsstudio | Studio for Composition](https://icst-kompositionsstudio.ch/post/icst-kompositionsstudio)
