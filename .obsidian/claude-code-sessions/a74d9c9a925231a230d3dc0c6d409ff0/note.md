...
5. **OSC Inputs & JavaScript** – Receives OSC messages and allows custom JavaScript for parameter control
6. **OSC Output** – Sends encoder state as OSC messages to external applications
...
7. **Distance Scaler** – Simulates distance perception via lowpass filtering and Doppler effect
8. **Infiniti** – Enables infinite distance mode, placing sources at the far field boundary
9. **Gain / Volume** – Adjusts the input or output gain of the encoder
10. **Import & Export** – Imports or exports source configurations as files
11. **Groups Editor** – Manages source groups; each group can be repositioned relative to its group center
...
12. **Save & Load Presets** – Stores and recalls complete encoder configurations
