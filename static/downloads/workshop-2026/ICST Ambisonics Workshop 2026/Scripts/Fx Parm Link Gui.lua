...
local src_fx = 0
local src_param = 0
local src_fx_names, src_param_names = {}, {}
...
local dst_fx = 0
local dst_param = 0
local dst_fx_names, dst_param_names = {}, {}
...
local function update_param_list_for(track_idx, fx_idx)
  ...
  for i = 0, pcount-1 do
    ...
    pnames[i] = "["..i.."] "..pname  -- Index mit anzeigen
  end
...
local function link_params()
  local dtrack = reaper.GetTrack(0, dst_track)
  local strack = reaper.GetTrack(0, src_track)
  -- get actual source track index (0-based)
  local src_tr_index = math.floor(reaper.GetMediaTrackInfo_Value(strack, "IP_TRACKNUMBER")) - 1

  reaper.ShowConsoleMsg("Linking src_param index: "..tostring(src_param).." to dest_param index: "..tostring(dst_param).."\n")

  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.active", "1")
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.src_tr", tostring(src_tr_index))
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.src_fx", tostring(src_fx))
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.src_param", tostring(src_param))
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.scale", "1")
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.min", "0")
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "param."..dst_param..".plink.max", "1")

  -- refresh the FX after setting parms
  reaper.TrackFX_SetNamedConfigParm(dtrack, dst_fx, "", "")
  reaper.TrackList_AdjustWindows(false)
  reaper.UpdateArrange()

  reaper.ShowMessageBox("Linked! Check ReaScript console for debug info.", "Done", 0)
end
