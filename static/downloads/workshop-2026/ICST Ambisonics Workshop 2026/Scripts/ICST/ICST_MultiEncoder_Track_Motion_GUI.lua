...
--------------------------------------------------
-- Apply motion per track / 16 sources
--------------------------------------------------
...
local fxIndex = reaper.TakeFX_AddByName(
  take,
  "ICST Ambisonics MultiEncoder",
  0,    -- normal Take FX (NICHT boolean!)
  -1    -- search only, do not insert
)
    ...
    if fxIndex == -1 then
      ...
      reaper.ShowMessageBox(
        "ICST MultiEncoder fehlt auf Track:\n" .. name,
        "ICST Animator",
        0
      )
      goto continue
    ...
    for s = 0, sources - 1 do
      ...
      local env_az = reaper.TakeFX_GetEnvelope(take, fxIndex, az_param, true)
      local env_el = reaper.TakeFX_GetEnvelope(take, fxIndex, el_param, true)
  ...
  reaper.PreventUIRefresh(-1)
  reaper.Undo_EndBlock("ICST Track Motion – 16 Sources", -1)
  reaper.UpdateArrange()
end

--------------------------------------------------
-- GUI Loop (FIXED Begin/End)
--------------------------------------------------
local function loop()
  ...
  if visible then
    reaper.ImGui_Text(ctx, "ICST MultiEncoder – 16 Source Motion")
